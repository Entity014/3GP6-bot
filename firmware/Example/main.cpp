...
PID joint1_pid(60.0, 120.0, KP_JOINT1, KI_JOINT1, KD_JOINT1);
PID joint2_pid(60.0, 120.0, KP_JOINT2, KI_JOINT2, KD_JOINT2);
...
void moveBase()
{
  ...
  debug_encoder_msg.angular.x = current_rpm3;
  debug_encoder_msg.angular.y = current_rpm4;
...
{
  tcaSelect(0);
  Arm_Kinematics::angle arm_joint, req_joint;
  Arm_Kinematics::position arm_position;
  // req_joint = arm_kinematics.getAngle();

  arm_joint.joint1 = DEG_TO_RAD * 90;
  arm_joint.joint2 = DEG_TO_RAD * 0;
  arm_joint.joint3 = DEG_TO_RAD * 90;
...
void readSensor()
{
  debug_joint_msg.linear.x = analogRead(JOINT1_SENSOR);
  debug_joint_msg.linear.y = analogRead(JOINT2_SENSOR);
}
...
void moveESC(int num, int degree)
{
  servo_array.setPWM(num, 0, map(degree, 0, 180, ESCMIN, ESCMAX));
}