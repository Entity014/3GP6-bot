        ...
        try:
            ...
            for i in range(len(slice_intervals) - 1):
                ...
                if nearest_contour is not None:
                        ...
                        centroids.append(
                            {
                                "coordinates": (cx, cy),
                                "area": area,
                                "distance": int(cx - (cv_image.shape[1] / 2)),
                            }
                        )
                    ...
                    # cv2.drawContours(cv_image, nearest_contour, -1, (0, 255, 0), 2)

            mask = cv2.inRange(
                hsv_image,
                hsv_threshold_dict["blue"][0],
                hsv_threshold_dict["blue"][1],
            )
            ...
            if len(centroids) > 1:
                ...
                cv2.line(cv_image, (cx1, cy1), (cx2, cy2), (255, 0, 0), 2)

                # print(centroids[0]["distance"] - angle_deg)

            cv2.imshow("Contours on Image", cv_image)
            cv2.waitKey(1)
